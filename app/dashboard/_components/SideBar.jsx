...
const SideBar = () => {
    ...
    return (
        <div className='fixed h-full md:w-64 p-5 bg-[#0d0d14] border-r border-border/50 flex flex-col'>
            <div className='mb-6'>
                {/* logo for the dashboard which redirect to dashboard on clicking */}
            ...
            <ul className='flex-1 space-y-1'>
                {Menu.map((item, idx) => (
                    item.name === 'Logout' ? (
                        ...
                        <li key={idx}>
                            <Link href={item.path}>
                                {/* is the current path (given by item.path) matches the path then it apply custom css */}
            ...
            <div className='mt-auto pt-4 border-t border-border/30'>
                {/* tracks the limit of course creation - valid because userCourseList is global context so it is consistent */}
