...
const opts = {
    height: '390', //height of the video player
    width: '640', //width of the video player
    playerVars: {
...
function ChapterContent({ chapter, content, isCompleted, onMarkCompleted }) {
    ...
    const handleMarkCompleted = async () => {
        ...
        try {
            ...
            {/* updating the parent component using the callback function onMarkCompleted which updates the dbChapters state on the basis of whiich the 
            progress tracker and all works properly */}
    ...
    if (typeof cleanCode === 'string') {
        {/* regex which hunts for opening and closing backticks and remove then and trim finally */ }
        cleanCode = cleanCode.replace(/```(html|css|js|javascript|jsx)?/gi, '').replace(/```/g, '').trim();
    }

    return (
        <div className='p-6 md:p-10 max-w-4xl mx-auto'>
            {/* displaying the chapter name and discription */}
                    ...
                    <div className='flex flex-col items-center w-full'>
                        {!videoError ? (
                            ...
                            <div className='bg-[#181818] p-8 rounded-xl border border-border/30 text-center max-w-xl w-full'>
                                {/* the video player fails to load then display a message and a button to watch the video on youtube */}
            ...
            {/* 3. Code Example (Conditionally rendered only if it contains actual code) and includes regex for 
        removing the html tags returned by the ai response and the trim() */}
